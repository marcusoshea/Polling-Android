            ...
            RetrofitInstance.api.login(loginRequest).enqueue(object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        ...
                        sharedPreferences.edit().putString("Name", pollingOrderMember.name).apply()
                        ...
                        Toast.makeText(context, "Login successful: ${sharedPreferences.getString("Name", null)}", Toast.LENGTH_SHORT).show()
                    } else {
